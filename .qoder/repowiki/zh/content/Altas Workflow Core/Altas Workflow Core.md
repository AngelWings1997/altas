...
- [SKILL.md](file://altas-workflow/references/superpowers/test-driven-development/SKILL.md)
- [SKILL.md](file://altas-workflow/references/checkpoint-driven/SKILL.md)
- [SKILL.md](file://altas-workflow/references/superpowers/systematic-debugging/SKILL.md)
- [SKILL.md](file://altas-workflow/references/superpowers/implementation-verify/SKILL.md)
- [SKILL.md](file://altas-workflow/references/superpowers/receiving-code-review/SKILL.md)
- [SKILL.md](file://altas-workflow/references/superpowers/requesting-code-review/SKILL.md)
- [SKILL.md](file://altas-workflow/references/superpowers/verification-before-completion/SKILL.md)
- [IMPLEMENTATION-PLAN-v4.6.md](file://altas-workflow/docs/IMPLEMENTATION-PLAN-v4.6.md)
</cite>

## 更新摘要
**变更内容**
- 新增实现验证系统作为REVIEW阶段的必需门禁，强化Spec-代码一致性审查
- 更新工作流规范强调标准化代码审查流程
- 完善三轴评审机制，新增Spec-代码一致性作为独立轴
- 增强代码审查后的验证流程，确保需求完全实现
- 新增实现验证技能的详细规范和集成点
...
J --> K[RIPER-5.md]
J --> L[RIPER-DOC.md]
J --> M[PROTOCOL-SELECTION.md]
end
subgraph "参考资料"
A --> N[references/]
N --> O[spec-driven-development/]
N --> P[checkpoint-driven/]
N --> Q[special-modes/]
N --> R[superpowers/]
end
subgraph "自动化工具"
...
**更新** Altas Workflow 4.6 版本新增实现验证系统作为REVIEW阶段的必需门禁

Altas Workflow 采用独特的三轴评审体系，现已强化为四轴评审：
...
A4[轴4: PRD实现验证<br/>需求覆盖率/任务完成率/合同实现] --> R4{覆盖率≥80%?}
end
R1 --> |"FAIL"| BACK1[回到Research/Plan]
R2 --> |"FAIL"| BACK1
R3 --> |"有高风险未解决"| BACK2[回到Plan]
R4 --> |"<80%"| BACK3[回到Execute]
R1 --> |"PASS"| NEXT1[进入下一步]
R2 --> |"PASS"| NEXT2[进入下一步]
R3 --> |"无高风险"| NEXT3[进入下一步]
R4 --> |"≥80%"| NEXT4[进入下一步]
NEXT1 --> NEXT2 --> NEXT3 --> NEXT4 --> ARCHIVE[完成/Archive]
```
...
Altas Workflow 提供灵活的协议选择机制，根据不同场景推荐最适合的协议：
...
| **用户完全自主控制模式** | RIPER-5 | 每个模式切换需要用户显式信号，适用于严格审批场景 |
| **双模型协作** | SDD-RIPER-DUAL-COOP | 适用于 External Model 无法直接读取代码库的场景 |
| **纯文档撰写** | RIPER-DOC | 文档结构化整理场景 |

#### 协议关系说明

- **ALTAS Workflow (SKILL.md)** 是统一入口点，内部引用 SDD-RIPER-ONE 为主要协议
- **RIPER-5** 是 SDD-RIPER-ONE 的更严格子集（更保守，用户控制更多）
- **SDD-RIPER-DUAL-COOP** 是 SDD-RIPER-ONE 的扩展（增加双模型信任边界管理）

#### 会话中协议切换

| 从 | 到 | 触发条件 |
|----|---|---------|
| SDD-RIPER-ONE | RIPER-5 | 用户说"我想要手动审批每个阶段" |
| SDD-RIPER-ONE | SDD-RIPER-DUAL-COOP | 用户激活双模型模式（SCOUT 和 ARCHITECT 角色） |
| RIPER-5 | SDD-RIPER-ONE | 用户说"你可以自主执行" |
...
**重大更新** 4.6 版本引入完整的纪律强制机制

Altas Workflow 4.6 版本引入了三层纪律强制机制，确保规则严格执行：
...
Altas Workflow 4.6 版本引入了专门的入口来源整合文件 `sources.md`，用于承载入口层的"来源整合"说明：
...
Superpowers 模块提供了丰富的专业技能，现已集成实现验证系统：
...
**新增** 4.6 版本引入的规格质量评估体系

Altas Workflow 提供了完整的 Spec 质量度量标准，确保规格文档的质量和可执行性：
...
**新增** 4.6 版本完善的测试驱动开发适配

Altas Workflow 的 TDD 协议与 Spec 驱动开发深度集成：

#### Spec-aware TDD 执行流程

当工作在 Spec-Driven Development 工作流下时：

- **The Spec's Plan section defines WHAT, TDD defines HOW to verify it**
- 如果 Plan 已经指定确切的签名和契约，你的 RED 测试应该验证这些契约目前是否未满足
- 如果 Plan 只描述了高层行为，遵循标准 TDD 循环来设计和验证接口
- 如果你的测试揭示 Plan 的设计有问题，STOP 并先更新 Spec（Reverse Sync 规则）
- "删除现有代码"规则同样适用于当你在任何测试之前编写了实现代码的情况

#### TDD 执行约束

- **无测试不编码**：没有失败的测试就不要写生产代码
- **Red-Green-Refactor 循环**：严格遵循 TDD 三个阶段
- **测试先行**：测试通过证明代码正确，测试失败证明需要改进
- **无例外原则**：任何情况下都不能跳过 TDD 流程

**章节来源**
- [SKILL.md:351-360](file://altas-workflow/references/superpowers/test-driven-development/SKILL.md#L351-L360)

### 组件 H：系统化调试流程

**新增** 4.6 版本的压力场景调试框架

Altas Workflow 提供了完整的系统化调试流程，包括压力场景下的决策框架：

#### 四阶段调试流程

```mermaid
flowchart TD
A[问题发现] --> B[根因调查 Phase 1]
B --> C[模式分析 Phase 2]
C --> D[假设验证 Phase 3]
D --> E[实施修复 Phase 4]
B --> F[证据收集]
...
面对生产系统故障，需要在快速修复和系统化调查之间做出选择：

- **选项 A**: 完整遵循系统化调试流程（35+ 分钟）
- **选项 B**: 快速修复（5 分钟），事后调查
- **选项 C**: 最小化调查后快速修复
...
- **选项 A**: 删除所有临时代码，重新开始系统化调试
- **选项 B**: 采用"够用就好"的解决方案，事后调查
- **选项 C**: 进行快速调查后选择折中方案

**章节来源**
- [systematic-debugging/SKILL.md:16-45](file://altas-workflow/references/superpowers/systematic-debugging/SKILL.md#L16-L45)
- [systematic-debugging/test-pressure-1.md:1-59](file://altas-workflow/references/superpowers/systematic-debugging/test-pressure-1.md#L1-L59)
- [systematic-debugging/test-pressure-2.md:1-69](file://altas-workflow/references/superpowers/systematic-debugging/test-pressure-2.md#L1-L69)

### 组件 I：实现验证系统

**新增** 4.6 版本的核心组件

实现验证系统作为REVIEW阶段的必需门禁，确保代码与Spec的完全一致性：
...
| 维度 | 检查内容 | 通过标准 |
|------|----------|----------|
| **PRD覆盖率** | FR-XXX需求与任务完成情况匹配 | ≥80%覆盖率 |
| **任务完成率** | tasks.md中任务完成情况 | 100%完成 |
| **合同实现** | API接口定义与实际实现匹配 | 100%实现 |
| **测试覆盖** | 指定测试与实际测试匹配 | 完全覆盖 |
...
participant Verify as 实现验证
participant Spec as 规格文档
participant Code as 代码库
Review->>Verify : 审查完成通知
Verify->>Spec : 读取PRD/spec
Verify->>Code : 检查实现
Verify->>Verify : 生成覆盖率报告
Verify->>Review : 返回验证结果
Review->>Review : 根据结果决定下一步
```

**图表来源**
- [SKILL.md:111-120](file://altas-workflow/references/superpowers/implementation-verify/SKILL.md#L111-L120)

**章节来源**
- [SKILL.md:1-218](file://altas-workflow/references/superpowers/implementation-verify/SKILL.md#L1-L218)

### 组件 J：标准化代码审查流程

**新增** 4.6 版本的审查流程标准化

Altas Workflow 现已建立标准化的代码审查流程，确保审查质量和效率：
...
| 维度 | 审查重点 | 通过标准 |
|------|----------|----------|
| **技术正确性** | 代码是否符合架构要求 | 无重大架构问题 |
| **语言规范性** | 符合语言最佳实践 | 无语言特定问题 |
| **需求实现性** | 是否满足PRD要求 | 100%需求覆盖 |
| **测试完备性** | 是否有充分测试 | 测试通过且覆盖充分 |

**章节来源**
- [SKILL.md:1-304](file://altas-workflow/references/superpowers/receiving-code-review/SKILL.md#L1-L304)
...
A --> Q[RIPER-5.md]
A --> R[RIPER-DOC.md]
A --> S[PROTOCOL-SELECTION.md]
A --> T[SDD-RIPER-DUAL-COOP.md]
end
subgraph "验证依赖"
B --> U[implementation-verify/SKILL.md]
B --> V[receiving-code-review/SKILL.md]
B --> W[requesting-code-review/SKILL.md]
B --> X[verification-before-completion/SKILL.md]
end
```
...
| **实现验证失败** | 覆盖率低于80% | 返回执行阶段完善实现 |

### 异常恢复策略

**重大更新** 4.6 版本将异常与恢复程序移至专门文件
...
Altas Workflow 强化了检查点机制，确保每个步骤都有明确的暂停点：

#### 强制暂停场景

- **批量执行前**：必须输出检查点，确认当前状态和预期结果
- **关键决策点**：涉及架构变更、跨项目修改、高风险操作时必须暂停
- **规格变更时**：任何对 Spec 的修改都需要用户确认
- **测试失败时**：必须暂停分析失败原因，制定修复计划
- **实现验证失败时**：覆盖率低于80%必须暂停，返回执行阶段
...
**章节来源**
- [checkpoint-driven/SKILL.md:58-68](file://altas-workflow/references/checkpoint-driven/SKILL.md#L58-L68)

### 批量执行回滚机制

**新增** 4.6 版本的批量操作安全保障

Altas Workflow 为批量执行提供了完善的回滚机制：

#### 回滚触发条件

- **测试失败**：批量执行后测试失败，立即触发回滚
- **规格冲突**：批量操作与现有规格产生冲突
- **风险升级**：批量操作引入不可接受的风险
- **用户撤销**：用户明确要求撤销批量操作
- **实现验证失败**：批量操作后实现验证失败
...
**图表来源**
- [refactor.md:137-145](file://altas-workflow/references/special-modes/refactor.md#L137-L145)
...
实现验证系统作为REVIEW阶段的必需门禁，确保需求完全实现：
...
**图表来源**
- [SKILL.md:100-108](file://altas-workflow/references/superpowers/implementation-verify/SKILL.md#L100-L108)
...
Altas Workflow 通过整合 SDD-RIPER、Checkpoint-Driven 和 Superpowers 三大工作流的优势，为企业级 AI 编程提供了完整的解决方案。4.6 版本的重大更新进一步强化了工作流的标准化和规范化：

### 主要优势

1. **工程化程度高**：通过严格的门禁机制和四轴评审，确保代码质量和可维护性
...
12. **协议选择灵活**：提供基于场景的协议选择指南，适应不同工作环境
13. **规格质量保障**：完整的规格质量度量标准，确保规格文档的可执行性
14. **TDD 深度集成**：与 Spec 驱动开发深度结合，确保测试驱动的正确实施
15. **调试能力完善**：提供系统化调试流程和压力场景决策框架
16. **实现验证门禁**：新增实现验证系统，确保需求完全实现
17. **标准化审查流程**：建立标准化的代码审查流程，提高审查质量和效率
18. **四轴评审强化**：将Spec-代码一致性作为独立评审轴，确保实现质量
...
- **代码审查**：通过标准化流程进行系统化质量把控
- **性能优化**：通过 PERF 模式进行系统化性能改进
- **技术债务清理**：通过 REFACTOR 模式进行系统化重构
- **迁移升级**：通过 MIGRATE 模式进行系统化迁移管理
- **方法论传播**：通过 sources.md 统一对外说明来源整合
- **协议选择**：通过 PROTOCOL-SELECTION.md 选择最适合的协议
- **规格质量评估**：通过 spec-quality-metrics.md 量化评估规格质量
- **系统化调试**：通过 systematic-debugging 提供压力场景下的决策框架
- **实现验证**：通过 implementation-verify 确保需求完全实现

### 发展前景

Altas Workflow 4.6 代表了 AI 原生研发的发展方向，通过将人工智能与工程实践深度融合，为企业数字化转型提供了强有力的技术支撑。4.6 版本的重大重构进一步强化了纪律强制机制，新增的实现验证系统和标准化代码审查流程使其在复杂企业环境中具备了更强的适应性和可靠性。协议选择指南、规格质量度量标准、TDD 适配规则和批量执行回滚机制等新特性，使得 Altas Workflow 在确保代码质量的同时，也提高了开发效率和团队协作水平。
...
| **规格质量评估** | `spec-quality-metrics.md` | 规格质量度量标准 |
| **实现验证系统** | `implementation-verify/SKILL.md` | PRD覆盖率检查 |
| **代码审查流程** | `receiving-code-review/SKILL.md` | 审查反馈处理 |
| **审查请求** | `requesting-code-review/SKILL.md` | 审查请求管理 |
| **验证门禁** | `verification-before-completion/SKILL.md` | 验证前门禁 |
...
| **实现验证** | `implementation-verify` | 需求验证 | PRD覆盖率检查 |
...
Altas Workflow 4.6 版本引入了全新的统一触发词字典系统，实现了以下改进：
...
Altas Workflow 4.6 版本引入了专门的入口来源整合文件，用于承载入口层的"来源整合"说明：

#### 来源整合内容

| 来源 | 采纳能力 |
|------|----------|
| **SDD-RIPER** | Spec 中心论、RIPER 状态机、四轴 Review、Multi-Project、Debug/Archive 协议 |
| **Checkpoint-Driven** | 轻量模式、4 级规模、Done Contract、Resume Ready、Hot/Warm/Cold 上下文策略 |
| **Superpowers** | TDD 铁律、系统化 Debug、Subagent、并行 Agent、验证优先、实现验证系统 |
...
为了确保路由决策的透明性和一致性，Altas Workflow 4.6 引入了快速判定树：
...
为了应对突发情况和紧急任务，Altas Workflow 4.6 引入了紧急响应协议：
...
**新增** 4.6 版本的 SKILL-entry-review.md 扩展
...
| 维度 | 输入 | 方法 | 输出 |
|------|------|------|------|
| **PRD覆盖率** | spec.md (FR-XXX) + tasks.md | 跨越PRD与任务完成情况 | 覆盖率百分比 |
| **任务完成率** | tasks.md | 统计[X]/[x] vs [ ]项 | 完成率百分比 |
| **合同实现** | contracts/ 目录 | 解析端点定义vs实际代码 | 合同覆盖率 |
| **测试覆盖对齐** | spec.md §Test Strategy + 实际测试 | 验证测试文件覆盖指定场景 | 缺口分析 |
...
| 代码 | 状态 | 含义 | 操作 |
|------|------|------|------|
| 0 | Complete | 100%实现 | 继续 |
| 1 | Partial | >80%实现 | 用户决定 |
| 2 | Low | <80%实现 | 阻止，返回执行 |
| 3 | Error | 缺少必要文件 | 修复环境 |
