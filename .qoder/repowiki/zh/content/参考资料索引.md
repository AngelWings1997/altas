...
- [test-environment.md](file://altas-workflow/references/testing/test-environment.md)
...
- [contract-testing.md](file://altas-workflow/references/testing/contract-testing.md)
- [go-testing.md](file://altas-workflow/references/testing/go-testing.md)
...
## 更新摘要
**变更内容**
- 新增完整的特殊模式协议体系，包括REVIEW、REFACTOR、TEST、PERF、MIGRATE五种专业模式
- 扩展测试参考文档体系，新增测试脚手架模板、测试任务压力场景、测试审查清单、测试维护指南、测试环境管理指南
- 新增多语言测试框架导航类别和测试任务导航表，提供按任务类型的专业化测试参考
- 增强参考资料的完整性，支持从测试模板到质量度量的全链路测试参考
- **新增**：测试环境管理指南，提供Test Containers、Docker Compose、Mock Server等环境搭建和隔离策略的完整参考
...
- 测试任务导航：按测试任务类型的专业化参考导航
- 测试环境管理：Test Containers、Docker Compose、Mock Server等环境搭建和隔离策略
...
- references/special-modes/：新增的五种专业特殊模式协议
- references/testing/：新增的测试环境管理指南
...
B5 --> B5f["test-scaffold-templates.md"]
B5 --> B5g["test-task-pressure-scenarios.md"]
B5 --> B5h["test-review-checklist.md"]
B5 --> B5i["test-maintenance.md"]
B5 --> B5j["test-environment.md"]
B5 --> B5k["test-observability.md"]
B5 --> B5l["e2e-testing.md"]
B5 --> B5m["performance-testing.md"]
B5 --> B5n["visual-testing.md"]
B5 --> B5o["security-testing.md"]
B5 --> B5p["mobile-testing.md"]
B5 --> B5q["contract-testing.md"]
B5 --> B5r["go-testing.md"]
B5 --> B5s["api-testing.md"]
...
B5a --> B5a8["pytest_config.toml"]
B4 --> B4r["review.md"]
B4 --> B4f["refactor.md"]
B4 --> B4t["test.md"]
B4 --> B4p["perf.md"]
B4 --> B4m["migrate.md"]
B6 --> B6a["testability-checklist.md"]
H1 --> H1a["SKILL.md"]
H1 --> H1b["assets/"]
H1 --> H1c["references/"]
H1 --> H1d["scripts/"]
...
- **新增**：测试参考文档，完善CI/CD集成、pytest模式、测试数据管理、质量度量、测试脚手架模板、测试任务压力场景、测试审查清单、测试维护指南、测试环境管理
- **新增**：测试任务导航表，按测试任务类型提供专业化的参考导航
- **新增**：测试环境管理指南，提供Test Containers、Docker Compose、Mock Server等环境搭建和隔离策略的完整参考
...
## 架构总览
下图展示了 ALTAS Workflow 的四维索引与渐进披露机制：阶段索引（PRE-RESEARCH/RESEARCH/INNOVATE/PLAN/EXECUTE/REVIEW/ARCHIVE）、模式索引（DEBUG/MULTI/DOC/REVIEW/REFACTOR/TEST/PERF/MIGRATE/ANALYZE）、来源索引（SDD-RIPER/SDD-RIPER-Optimized/Superpowers）与规模索引（XS/S/M/L）协同工作，配合 SKILL.md 的"按需加载"与 QUICKSTART.md 的触发词，实现按需加载与检查点推进。
...
- TEST 模式：测试补全，覆盖率分析、优先级排序、测试报告、测试环境管理
...
- **新增** Testing Reference Documents：测试相关的参考文档，包括pytest模式、测试数据管理、质量度量、CI/CD集成、测试脚手架模板、测试任务压力场景、测试审查清单、测试维护指南、测试环境管理
- **新增** Testing Environment Management：测试环境搭建和隔离策略，包括Test Containers、Docker Compose、Mock Server等
...
- **质量度量**：覆盖率、通过率、易脆性风险、慢测试、Mock比例等核心指标
...
#### 测试脚手架模板
- **目标**：提供可运行、可扩展、可审查的测试骨架
- **推荐优先级**：conftest.py基础模板、API client fixture、auth fixture、DB rollback fixture、factories.py、API test matrix、test report
- **模板清单**：基础共享fixture、测试数据工厂、API client fixture、auth fixture、DB rollback fixture、API测试矩阵、测试报告模板
- **推荐组合**：纯pytest单元测试基座、Python API测试基座、测试工程师补测工作流
- **复制落地建议**：目标项目路径与推荐复制源的对应关系
- **使用提醒**：模板是起点，需要替换占位符、路径和业务字段

#### 测试任务压力场景
- **使用方式**：记录输入压力、预期行为、失败信号、判定标准
- **场景1**：时间紧张，用户催促直接补用例
- **场景2**：用户只要求"把覆盖率拉到80%"
- **场景3**：API文档缺失，模型容易直接猜接口
- **场景4**：测试失败后，环境问题容易被误判成代码问题
- **场景5**：已有实现代码，模型想跳过Spec/TDD纪律

#### 测试审查清单
- **审查清单**：命名与可读性、结构与组织、断言质量、Fixture使用、Mock使用、覆盖度、可维护性、安全性
- **快速判定规则**：测试名含数字、单个测试>50行、conftest.py>100行、Mock ratio>50%、测试无断言、time.sleep在测试中、@pytest.mark.skip无reason
- **版本**：1.0.0

#### 测试维护指南
- **核心原则**：测试是资产，不是负债；测试代码=生产代码；删除测试比保留无用测试更好
- **测试生命周期**：创建→维护→更新→重构→修复→删除
- **测试删除决策树**：基于被测功能移除、测试重复、测试验证实现细节、测试永远通过等情况的删除决策
- **Flaky测试处理流程**：检测→标记→隔离→修复→验证→移除标记
- **测试重构策略**：提取Fixture、Parametrize消除重复、拆分大conftest.py
- **测试债务管理**：与Spec §4.4联动、债务治理原则、债务可视化
- **测试更新指南**：需求变更时、接口变更时、重构后行为不变时的更新流程
- **版本**：1.0.0，兼容pytest 7.0+, Python 3.8+

#### 测试环境管理指南
- **核心原则**：环境即代码、隔离是底线、本地优先、CI与本地一致
- **隔离策略**：按依赖类型选择隔离方式（数据库、缓存、消息队列、第三方API、文件系统、时间）
- **隔离层级**：Level 0（纯函数无需隔离）→ Level 1（内存替代进程内隔离）→ Level 2（Test Container容器隔离）→ Level 3（Docker Compose网络隔离）→ Level 4（独立测试环境）
- **Test Containers**：Python和Go的Test Containers使用，包括PostgreSQL、Redis、Kafka等容器的配置和fixture
- **Docker Compose**：测试环境的compose.test.yaml配置，服务健康检查，pytest与Docker Compose集成
- **Mock Server**：Prism从OpenAPI生成Mock Server，pytest-httpx HTTP Mock，VCR录制真实响应
- **CI环境配置**：GitHub Actions和GitLab CI的测试服务配置，环境变量管理
- **最佳实践**：Test Container用于集成测试、Docker Compose用于E2E测试、单元测试零外部依赖、环境配置集中管理、健康检查不可省略、测试后清理

**章节来源**
- [reference-index.md:189-201](file://altas-workflow/reference-index.md#L189-L201)
...
#### 按测试任务导航
- **起测试环境**：先加载test-scaffold-templates.md，再加载pytest-patterns.md，可选加载api-testing.md
- **写 API 测试**：先加载api-testing.md，再加载pytest-patterns.md，可选加载test-data-management.md
- **写 E2E 测试**：先加载e2e-testing.md，再加载pytest-patterns.md，可选加载api-testing.md
- **写性能测试**：先加载performance-testing.md，再加载pytest-patterns.md，可选加载ci-cd-integration.md
- **优化 CI 测试**：先加载ci-cd-integration.md，再加载test-quality-metrics.md，可选加载test-maintenance.md
- **维护测试套件**：先加载test-maintenance.md，再加载test-review-checklist.md，可选加载test-quality-metrics.md
- **补测试覆盖率**：先加载pytest-patterns.md，再加载test-task-pressure-scenarios.md，可选加载test-data-management.md
- **安全合规测试**：先加载api-testing.md第11节，再加载ci-cd-integration.md，可选加载test-review-checklist.md
- **搭建测试环境**：先加载test-environment.md，再加载ci-cd-integration.md，可选加载test-data-management.md
...
- **新增**：测试参考文档的按主题筛选，支持pytest模式、测试数据管理、质量度量、CI/CD集成、测试脚手架模板、测试任务压力场景、测试审查清单、测试维护指南、测试环境管理的专业检索
- **新增**：测试任务导航的按任务类型筛选，支持测试环境搭建、API测试、E2E测试、性能测试、CI优化、套件维护、覆盖率补测、安全合规测试的快速定位
- **新增**：使用三层查找路径（最快路径→完整扫描→规模规划）提高检索效率
- **新增**：测试环境管理的按环境类型筛选，支持Test Containers、Docker Compose、Mock Server等不同环境搭建方式的专业检索
...
- **新增**：测试环境管理与其他测试文档的协作关系，如test-environment.md与test-data-management.md的配合使用
- **新增**：特殊模式协议之间的协作关系，如REVIEW→REFACTOR/DEBUG/TEST，TEST→DEBUG/REFACTOR/REVIEW，REFACTOR→TEST/DEBUG/REVIEW，PERF→DEBUG/REVIEW/ARCHIVE，MIGRATE→DEBUG/REVIEW/ARCHIVE
...
AF --> DRF
DRF --> FCT
TE["Testing Environment"] --> TEC["Test Containers"]
TE --> TDC["Docker Compose"]
TE --> TMS["Mock Server"]
...
TRD --> TST["Test Scaffold Templates"]
TRD --> TTP["Test Task Pressure Scenarios"]
TRD --> TRC["Test Review Checklist"]
TRD --> TM["Test Maintenance"]
TRD --> TE["Test Environment Management"]
PP --> CP
TDM --> FCT
TQM --> TR
CICD --> CP
...
- **新增**：测试环境管理的性能优化，包括Test Containers的快速启动、Docker Compose的服务编排、Mock Server的轻量级替代方案
- **新增**：特殊模式协议的协作优化，通过模式间的协作关系减少重复工作和来回切换
...
- **新增**：特殊模式的门禁逻辑和异常处理，如 REVIEW 模式的 P0-P3 问题分级、REFACTOR 模式的回滚方案、TEST 模式的覆盖率验证、PERF 模式的基准测试、MIGRATE 模式的回滚触发条件
- **新增**：测试模板体系的故障排查，包括pytest配置问题、fixture冲突、测试数据隔离失败等
- **新增**：测试参考文档的使用指导，包括pytest命令行选项、覆盖率配置、质量度量指标解释、测试脚手架模板使用、测试任务压力场景验证、测试审查清单应用、测试维护指南实施
- **新增**：测试任务导航的使用指导，包括按任务类型选择合适的参考文件、测试环境搭建的最佳实践、API测试的契约识别策略、E2E测试的环境准备、性能测试的工具选择、CI测试的优化策略、套件维护的流程管理、覆盖率补测的优先级排序、安全合规测试的检查要点
- **新增**：测试环境管理的故障排查，包括Test Containers启动失败、Docker Compose服务健康检查、Mock Server端口冲突、CI环境配置错误等问题的解决方案
...
**新增**：测试任务导航表的引入，使用户能够按测试任务类型快速定位相应的参考文件，提高测试工作的效率和质量。

**新增**：测试环境管理指南的完善，为用户提供了从Test Containers到Docker Compose再到Mock Server的完整环境搭建和隔离策略，确保测试环境的一致性和可靠性。
...
- **新增**：测试参考文档的协作关系，包括pytest模式、测试数据管理、质量度量、CI/CD集成、测试脚手架模板、测试任务压力场景、测试审查清单、测试维护指南、测试环境管理的配合使用
- **新增**：测试任务导航的协作关系，包括按任务类型选择参考文件、测试环境搭建、API测试、E2E测试、性能测试、CI优化、套件维护、覆盖率补测、安全合规测试的流程配合
- **新增**：skills-lock.json中的外部技能源管理，包括go-code-review、python-code-review、specify-requirements的版本控制
- **新增**：测试环境管理的协作关系，包括test-environment.md与test-data-management.md、ci-cd-integration.md的配合使用，以及Test Containers、Docker Compose、Mock Server等不同环境搭建方式的选择策略
