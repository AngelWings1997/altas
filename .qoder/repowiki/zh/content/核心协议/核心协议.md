...
## 简介
本文件面向 ALTAS Workflow 的核心协议，系统化解析 SKILL.md 4.4 版本的重大更新，重点阐述：
...
**更新** 本版本引入了 Persona 设定、路由冲突解决优先级、工具映射规则等核心特性，显著提升了协议的智能化水平和执行效率。

## 项目结构
ALTAS Workflow 采用"主协议 + 统一引用索引 + 按需模块"的组织方式：
- **主协议**：SKILL.md 4.4 版本，定义整体流程、触发词、规模评估、铁律与输出规范
...
```mermaid
graph TB
主协议["altas-workflow/SKILL.md<br/>4.4 版本 - 增强核心协议"]
...
### Persona 设定与角色定位
**更新** SKILL.md 4.4 版本新增了详细的 Persona 设定，明确了高级软件工程师的角色定位。
...
```mermaid
graph TB
主协议["altas-workflow/SKILL.md<br/>4.4 版本"]
...
## 结论
ALTAS Workflow 4.4 版本通过 Persona 设定、路由冲突解决机制、工具映射规则、任务分解要求、问题升级机制等核心特性，显著提升了协议的智能化水平和执行效率。新版本的入口瘦身理念通过统一引用索引实现了更好的模块化和可维护性，开发者应遵循铁律约束，按需加载参考文件，严格执行检查点输出与三轴审查，确保交付质量与可维护性。
