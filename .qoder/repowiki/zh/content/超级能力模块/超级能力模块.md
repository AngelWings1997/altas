...
## 项目结构
超级能力模块位于 `altas-workflow/references/superpowers/` 目录下，围绕五大能力域构建：
...
## 核心组件
本节聚焦“超级能力”的七大关键能力及其在 ALTAS Workflow 中的应用位置与约束。
...
## 架构总览
下图展示 ALTAS Workflow 的总体架构与超级能力的交互关系：设计与规划（头脑风暴、计划写作）→ 开发与验证（TDD、完成前验证）→ 并行与协作（Subagent、审查）→ 调试与修复（系统化 Debug）→ 辅助工具（并行 Agent、Git Worktree、分支完成）。
...
## 结论
超级能力模块以“证据优先、过程可审计、结果可验证”为核心，将设计、开发、调试、协作与收尾串联为闭环。通过 TDD 铁律、系统化 Debug、Subagent 驱动开发、计划写作与完成前验证等能力，开发者可在不同规模任务中稳定提升交付质量与效率。建议在团队内统一遵循铁律与门禁，结合流程图与参考索引，持续优化执行节奏与质量门控。
