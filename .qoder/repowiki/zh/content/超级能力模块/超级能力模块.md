...
- [altas-workflow/references/skills-lock.json](file://altas-workflow/references/skills-lock.json)
</cite>
...
- 分支完成管理：合并与收尾
- 技能编写：指南与范式
- **技能锁定机制**：基于 skills-lock.json 的技能版本管理、来源追踪和哈希验证
...
- **技能锁定机制**
  - **功能**：管理 workflow skills 及其依赖关系，提供版本控制、来源追踪和哈希验证
  - **作用**：确保技能的一致性和可追溯性，防止技能冲突和版本漂移
  - **应用**：在技能创建、分发和执行过程中提供完整性保障
...
## 架构总览
下图展示 ALTAS Workflow 的总体架构与超级能力的交互关系：设计与规划（头脑风暴、计划写作）→ 开发与验证（TDD、完成前验证）→ 并行与协作（Subagent、审查）→ 调试与修复（系统化 Debug）→ 依赖管理（技能锁定）→ 辅助工具（并行 Agent、Git Worktree、分支完成）。
...
subgraph "依赖管理"
L1["技能锁定<br/>skills-lock.json"]
end
subgraph "辅助工具"
U1["并行 Agent 派遣<br/>dispatching-parallel-agents"]
U2["执行计划非 Subagent<br/>executing-plans"]
U3["Git Worktree 管理<br/>using-git-worktrees"]
U4["完成分支<br/>finishing-a-development-branch"]
U5["编写技能<br/>writing-skills"]
end
B1 --> B2
B2 --> V1
V1 --> V2
...
### 技能锁定机制：版本管理与完整性保障
- **核心功能**：skills-lock.json 提供技能版本管理、来源追踪和哈希验证
- **版本管理**：记录每个技能的版本信息，确保技能的一致性和可追溯性
- **来源追踪**：跟踪技能的来源和依赖关系，防止技能冲突和版本漂移
- **哈希验证**：通过哈希值验证技能的完整性，确保技能在传输和存储过程中的准确性
- **应用场景**：
  - 技能创建时的版本控制
  - 技能分发时的来源验证
  - 技能执行时的完整性检查
...
- **技能锁定机制**通过哈希验证和版本控制减少技能冲突，提升系统稳定性
...
- **技能锁定异常**：版本冲突、哈希不匹配、来源不明
  - 处理：检查 skills-lock.json 的版本信息；验证技能哈希值；确认来源可信度
...
## 结论
超级能力模块以"证据优先、过程可审计、结果可验证"为核心，将设计、开发、调试、协作与收尾串联为闭环。通过 TDD 铁律、系统化 Debug、Subagent 驱动开发、计划写作与完成前验证等能力，开发者可在不同规模任务中稳定提升交付质量与效率。**新增的技能锁定机制**进一步强化了系统的可靠性，通过版本管理、来源追踪和哈希验证确保技能的一致性和完整性。建议在团队内统一遵循铁律与门禁，结合流程图与参考索引，持续优化执行节奏与质量门控。
...
- **技能锁定机制详情**：[altas-workflow/references/skills-lock.json](file://altas-workflow/references/skills-lock.json)