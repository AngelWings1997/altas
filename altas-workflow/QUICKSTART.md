...
如果你第一次用 ALTAS Workflow，先记住一件事：**直接把任务写成触发词命令发给 AI**。不需要先解释工作流原理，先发命令即可。
...
1. 判断任务大小，选一个触发词。
2. 把目标、范围、限制条件写清楚。
3. 如果有 Spec、日志、接口文档、原型图，就在同一条消息里明确让 AI 先读。
...
- 新增一个完整接口或模块
- 跨多个文件实现功能
- 需要明确 Research / Plan / Execute / Review
- 你希望 AI 严格按 Spec 驱动推进
...
ALTAS Workflow 最怕的不是任务复杂，而是**指令模糊**。下面这个模板通常最好用：
