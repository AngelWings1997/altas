...
- **变更留痕**：`AI_CHANGELOG.md`（可选，但强烈推荐；需在项目中自行创建）
...
> 举个教学化的虚构案例：某次复杂功能演练经历了多轮优化与 bugfix。后续又出现几个小 bug、需求追加了几个新点；如果不用 SDD，就得翻提交+人工回忆来重建上下文，既慢也不精确；如果用 SDD，只要把"新需求文档 + 旧实施文档/决策留痕"交给模型，通常 10 分钟内就能启动下一轮迭代。
...
- **动作**：在项目根目录维护一个 `.cursorrules` 文件或 `docs/skills/SKILL.md`（推荐结构，需在项目中创建）。
- **内容**：
...
- **红线**：**严禁**把敏感信息喂给外部大模型。
- **SOP**：先在受控环境中把原始信息整理为抽象 Spec，再让外部大模型基于抽象 Spec 做设计与推演，最后回到受控环境完成实现与验证。
...
- 可进一步参考公开的提示词工程资料，或结合团队内部规范整理自己的 SOP。
...
**第一步：准备"手脚" (脚本)** 创建一个简单的 Python 脚本 `scripts/log_change.py`（示例如下，需自行创建），用于标准化追加日志。
...
## 注：以上图片均由生成式模型辅助制作，内容经过整理与改写。

扩展阅读可直接从本仓库 README 导航进入。
