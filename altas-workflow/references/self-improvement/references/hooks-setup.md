    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "~/.claude/scripts/self-improvement-activator.sh"
          }
        ]
      }
    ]
...
```bash
chmod +x ./scripts/self-improvement-activator.sh
chmod +x ./scripts/error-detector.sh
chmod +x ./scripts/extract-skill.sh
```
...
```json
{
  "command": "/absolute/path/to/scripts/self-improvement-activator.sh"
}
```
